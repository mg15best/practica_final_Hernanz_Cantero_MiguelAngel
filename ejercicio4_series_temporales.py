...
def visualizar_serie(serie):
    ...
    ax.set_title("Serie temporal sintética completa")
    ax.set_xlabel("Fecha")
    ax.set_ylabel("Valor")
    ax.grid(alpha=0.3)
...
def descomponer_serie(serie):
    """
    Descompone la serie en Tendencia, Estacionalidad y Residuo usando
    seasonal_decompose y guarda el gráfico.
...
def analizar_residuo(residuo):
    ...
    plot_pacf(residuo_limpio, ax=axes[1], lags=40, method="ywm")
    ...
    plt.hist(residuo_limpio, bins=30, density=True, alpha=0.7)
    ...
    with open("output/ej4_analisis.txt", "w", encoding="utf-8") as f:
        ...
        f.write(f"p-value JB: {jb_p:.6f}\n")
        f.write("\n")
        f.write("Test ADF (estacionariedad)\n")
        f.write(f"Estadístico ADF: {adf_stat:.6f}\n")
        f.write(f"p-value ADF: {adf_p:.6f}\n")
        f.write("\n")

        if jb_p > 0.05:
            f.write("Conclusión normalidad: no se rechaza la normalidad.\n")
        else:
            f.write("Conclusión normalidad: se rechaza la normalidad.\n")

        if adf_p < 0.05:
            f.write("Conclusión ADF: el residuo es estacionario.\n")
        else:
            f.write("Conclusión ADF: no hay evidencia suficiente de estacionariedad.\n")
